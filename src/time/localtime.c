...
# define THREAD_RWLOCK 1
# define THREAD_TM_MULTI 1
# define THREAD_PREFER_SINGLE 1
#endif
...
#define lock __lcl_lock
#define unlock __lcl_unlock

/* Expose for compat libc12 */
#define rd2wrlock __lcl_rd2wrlock
int rd2wrlock(bool);
#define is_threaded __lcl_isthreaded
bool is_threaded(void);
...
void unlock(ATTRIBUTE_MAYBE_UNUSED bool threaded) { }
# endif
#endif
...
struct lsinfo {				/* leap second information */
	__time_t	ls_trans;	/* transition time (positive) */
...
static bool increment_overflow_time(__time_t *, int_fast32_2s);
static int_fast32_2s leapcorr(struct state const *, __time_t);
...
tzparse(const char *name, struct state *sp, struct state const *basep)
{
	...
	if (*name != '\0') {
		...
		*/
		...
		init_ttinfo(&sp->ttis[1], -dstoffset, true, (desigidx_type)(stdlen + 1));
		timecnt = 0;
		janfirst = 0;
		yearbeg = EPOCH_YEAR;
	 ...
	 struct tm *const tmp)
{
	...
	if ((sp->goback && t < sp->ats[0]) ||
		(sp->goahead && t > sp->ats[sp->timecnt - 1])) {
			...
			years = (time_t)(seconds / SECSPERREPEAT
			    * YEARSPERREPEAT);
			seconds = (time_t)(years * AVGSECSPERYEAR);
			years += YEARSPERREPEAT;
			if (t < sp->ats[0])
			  newt = (time_t)(t + seconds + SECSPERREPEAT);
			else
			  newt = (time_t)(t - seconds - SECSPERREPEAT);
       ...
       int_fast32_t offset, struct tm *tmp)
{
	...
	*/
	if (result)
		result->TM_ZONE = UNCONST(offset ? wildabbr
